  ...
  <div class="reservation-list">
        ...
        <div class="stat-card-rich total">
          <div class="stat-card-header">
            ...
            <div class="stat-card-trend up">
              <RiseOutlined />
              <span>{{ reservationStore.todayReservations }}</span>
            </div>
          </div>
          ...
          <div class="stat-card-footer">
            <span>今日新增 {{ reservationStore.todayReservations }} 条</span>
          </div>
        </div>
      </a-col>
      <a-col :xs="12" :sm="12" :md="6">
        <div class="stat-card-rich waiting">
          ...
          <div class="stat-card-body">
            <div class="stat-card-value">{{ reservationStore.activeReservations }}</div>
      ...
      <a-col :xs="12" :sm="12" :md="6">
        <div class="stat-card-rich fulfilled">
          ...
          <div class="stat-card-body">
            ...
            <div class="stat-card-label">已履约</div>
          </div>
          <div class="stat-card-footer">
            <a-progress
              ...
              size="small"
            />
            <span>履约率 {{ fulfilledPercent }}%</span>
          </div>
        </div>
      </a-col>
      <a-col :xs="12" :sm="12" :md="6">
        <div class="stat-card-rich cancelled">
          <div class="stat-card-header">
            ...
            <div class="stat-card-badge warning" v-if="cancelledCount > 0">
              <WarningOutlined />
            </div>
          </div>
          <div class="stat-card-body">
          ...
          <div class="stat-card-footer">
            <span v-if="cancelledCount > 0" class="warning-text">
              <AlertOutlined /> 请关注取消率
            </span>
      ...
      <a-row :gutter="16" align="middle">
        ...
        <a-col :xs="24" :sm="12" :md="8" :lg="6">
          ...
          >
            ...
            <a-select-option value="fulfilled">已履约</a-select-option>
      ...
      >
        <template #bodyCell="{ column, record, index }">
          ...
          <template v-else-if="column.key === 'position'">
            <div v-if="record.status === 'waiting'" class="position-cell">
              <a-badge :count="getPosition(record)" :number-style="{ backgroundColor: '#faad14' }" />
              <span class="position-text">第 {{ getPosition(record) }} 位</span>
            </div>
          ...
          <template v-else-if="column.key === 'action'">
            <a-space>
              <a-button
                ...
                type="link"
                size="small"
                class="table-action-btn fulfill-btn"
                @click="handleFulfill(record)"
                :disabled="!canFulfill(record)"
              >
                <CheckOutlined /> 立即借阅
              </a-button>
              <a-popconfirm
    ...
    >
      ...
      >
        ...
        <a-alert
          v-if="reserveForm.bookId && hasDuplicateWarning"
          message="该读者已预约此书，无法重复预约"
          type="warning"
          show-icon
...
const columns = [
  ...
  { title: '排队位置', key: 'position', width: 100 },
...
const waitingPercent = computed(() => {
  ...
  if (total === 0) return 0
  return Math.round((reservationStore.activeReservations / total) * 100)
})
...
const hasDuplicateWarning = computed(() => {
  if (!reserveForm.readerId || !reserveForm.bookId) return false
  return reservationStore.hasActiveReservation(reserveForm.bookId, reserveForm.readerId)
})
...
function getPosition(record) {
  return reservationStore.getReservationPosition(record.bookId, record.readerId)
}

function canFulfill(record) {
...
async function handleReserveSubmit() {
  try {
    ...
    if (reservationStore.hasActiveReservation(book.id, reader.id)) {
      message.error('该读者已预约此书，无法重复预约')
      return
    }
...
function handleFulfill(record) {
  ...
  reservationStore.fulfillReservation(record.id, borrowId)
...
.stat-card-rich {
  ...
  &.cancelled {
    ...
    .stat-card-icon { background: linear-gradient(135deg, #8c8c8c 0%, #a6a6a6 100%); }
